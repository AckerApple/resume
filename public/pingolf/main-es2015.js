...
let AppData = class AppData {
    constructor(db, Router, FireUser) {
        this.db = db;
        this.Router = Router;
        this.FireUser = FireUser;
        this.subs = new rxjs__WEBPACK_IMPORTED_MODULE_1__["Subscription"]();
        this.load = new Promise((res, rej) => {
            this.initResolver = res;
        });
        this.subs.add(this.FireUser.login.subscribe(user => {
            this.user = user;
            // console.log("user", user)
            // TIP: must come before loadPlayer to prevent never ending loop of updates      
            this.updatePlayerByUser(this.user);
            this.subs.add(this.db.loadPlayer(this.user.uid).subscribe(player => {
                this.player = player;
            }));
            this.resolveInit();
        }))
            .add(this.FireUser.logout.subscribe(user => {
            delete this.user;
            this.logout();
            this.resolveInit();
        }));
    ...
    updatePlayerByUser(user) {
        const sub = this.db.loadPlayer(this.user.uid).subscribe(player => {
            sub.unsubscribe();
            const up = this.db.userToPlayer(this.user);
            this.player = Object.assign({}, player, up);
            this.db.savePlayer(this.player); // updated save data
        });
    }
};
AppData.ctorParameters = () => [
...
/* harmony import */ var ack_angular_fire__WEBPACK_IMPORTED_MODULE_11__ = __webpack_require__(/*! ack-angular-fire */ "./node_modules/ack-angular-fire/dist/index.js");
/* harmony import */ var ack_angular_fire__WEBPACK_IMPORTED_MODULE_11___default = /*#__PURE__*/__webpack_require__.n(ack_angular_fire__WEBPACK_IMPORTED_MODULE_11__);
/* harmony import */ var ack_angular_fx__WEBPACK_IMPORTED_MODULE_12__ = __webpack_require__(/*! ack-angular-fx */ "./node_modules/ack-angular-fx/dist/index.js");
/* harmony import */ var ack_angular_fx__WEBPACK_IMPORTED_MODULE_12___default = /*#__PURE__*/__webpack_require__.n(ack_angular_fx__WEBPACK_IMPORTED_MODULE_12__);
/* harmony import */ var ack_angular_modules_router_AckRouterModule__WEBPACK_IMPORTED_MODULE_13__ = __webpack_require__(/*! ack-angular/modules/router/AckRouterModule */ "./node_modules/ack-angular/modules/router/AckRouterModule.js");
/* harmony import */ var ack_angular_modules_router_AckRouterModule__WEBPACK_IMPORTED_MODULE_13___default = /*#__PURE__*/__webpack_require__.n(ack_angular_modules_router_AckRouterModule__WEBPACK_IMPORTED_MODULE_13__);
/* harmony import */ var _db__WEBPACK_IMPORTED_MODULE_14__ = __webpack_require__(/*! ./db */ "./src/app/db.ts");
/* harmony import */ var _firebaseConfig__WEBPACK_IMPORTED_MODULE_15__ = __webpack_require__(/*! ./firebaseConfig */ "./src/app/firebaseConfig.ts");
/* harmony import */ var _app_routing__WEBPACK_IMPORTED_MODULE_16__ = __webpack_require__(/*! ./app.routing */ "./src/app/app.routing.ts");
/* harmony import */ var _AppData_class__WEBPACK_IMPORTED_MODULE_17__ = __webpack_require__(/*! ./AppData.class */ "./src/app/AppData.class.ts");
/* harmony import */ var _firebaseUiAuthConfig__WEBPACK_IMPORTED_MODULE_18__ = __webpack_require__(/*! ./firebaseUiAuthConfig */ "./src/app/firebaseUiAuthConfig.ts");
...
AppModule = tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_4__["NgModule"])({
        ...
        imports: [
            ...
            ack_angular_fire__WEBPACK_IMPORTED_MODULE_11__["Module"],
            ack_angular_fx__WEBPACK_IMPORTED_MODULE_12__["AckFxModule"],
            ack_angular_modules_router_AckRouterModule__WEBPACK_IMPORTED_MODULE_13__["AckRouterModule"].forRoot(),
            _angular_fire__WEBPACK_IMPORTED_MODULE_6__["AngularFireModule"].initializeApp(_firebaseConfig__WEBPACK_IMPORTED_MODULE_15__["firebaseConfig"], 'ackerapple'),
            firebaseui_angular__WEBPACK_IMPORTED_MODULE_10__["FirebaseUIModule"].forRoot(_firebaseUiAuthConfig__WEBPACK_IMPORTED_MODULE_18__["firebaseUiAuthConfig"]),
        ...
        providers: [_db__WEBPACK_IMPORTED_MODULE_14__["db"], _AppData_class__WEBPACK_IMPORTED_MODULE_17__["AppData"]],
...
AppRoutingModule = tslib__WEBPACK_IMPORTED_MODULE_0__["__decorate"]([
    Object(_angular_core__WEBPACK_IMPORTED_MODULE_3__["NgModule"])({
        imports: [
            ...
            ack_angular_AckModule__WEBPACK_IMPORTED_MODULE_10__["AckModule"].forRoot(),
...
let ScoreCard = class ScoreCard {
    constructor(db, AppData) {
        ...
        this.AppData.load
            .then(() => this.createDataSubs());
    }
    checkAppUserInGame() {
        this.appUserInGame = this.course && this.course.players.find(player => player.playerId === this.AppData.user.uid) ? true : false;
    }
    joinAppUser() {
        const player = this.db.userToPlayer(this.AppData.user);
        const dbPlayerRound = {
            player: player,
    ...
    createDataSubs() {
        this.db.loadCourse(this.courseId).subscribe(data => {
            this.dbData.course = data;
            this.reMarryCourseData();
        });
        this.db.loadCourseRoundPlayers(this.courseId, this.roundId).subscribe((data) => {
            this.dbData.courseRoundPlayers = data;
            this.reMarryCourseData();
            this.checkAppUserInGame();
            const matchedCoursePlayer = data.find(player => player.playerId === this.AppData.user.uid);
            this.roundEditor = matchedCoursePlayer && matchedCoursePlayer.roundEditor;
            this.checkEditPlayerHole();
        });
        this.db.loadMachines().subscribe(data => {
            this.dbData.machines = data;
            this.reMarryCourseData();
        });
    }
    checkEditPlayerHole() {
        ...
        const hole = this.course.holes.find(hole => hole.uuid === this.editPlayerHole.hole.uuid);
        const coursePlayer = this.course.players.find(coursePlayer => coursePlayer.playerId === this.editPlayerHole.coursePlayer.playerId);
        const playerHoleScore = coursePlayer.holes.find(playerScore => playerScore.holeId === hole.uuid);
    ...
    reMarryCourseData() {
        const { course, courseRoundPlayers, machines
        // , players
         } = this.dbData;
    ...
    getDbCourseRoundPlayer(playerId) {
        return this.dbData.courseRoundPlayers.find(crp => crp.playerId === playerId);
    }
    getPlayerRoundMeta(playerId) {
        return this.course.players.find(crp => crp.playerId === playerId);
    }
    saveCourseRoundPlayer(courseRoundPlayer) {
        const playerId = courseRoundPlayer.playerId;
        const dbCoursePlayer = this.getDbCourseRoundPlayer(playerId);
    ...
    savePlayerStrokes(playerId) {
        this.saveCourseRoundPlayerById(playerId);
    }
    saveCourseRoundPlayerById(playerId) {
        const courseRoundPlayer = this.getPlayerRoundMeta(playerId);
        this.saveCourseRoundPlayer(courseRoundPlayer);
    }
    deleteCourseRoundPlayer(courseRoundPlayer) {
        this.db.deleteCourseRoundPlayer(courseRoundPlayer);
    }
};
ScoreCard.ctorParameters = () => [
...
let db = class db {
    ...
    getCourseRoundPlayers(courseId, roundId) {
        return this.getPinGolf().collection("coursePlayers").doc(courseId).collection(roundId);
    }
    loadCourse(uid) {
        return this.getCourseDoc(uid).valueChanges();
    }
    loadCourseRoundPlayers(courseId, roundId) {
        const collect = this.getCourseRoundPlayers(courseId, roundId);
    ...
    saveCourseRoundPlayer(coursePlayer) {
        const { courseId, roundId, playerId } = coursePlayer;
        return this.getCourseRoundPlayers(courseId, roundId).doc(playerId).set(coursePlayer);
    }
    deleteCourseRoundPlayer(coursePlayer) {
        const { courseId, roundId, playerId } = coursePlayer;
    ...
    userToPlayer(user) {
        return {
            uuid: user.uid,
            name: (user.displayName || user["m"] || user.email),
...
function marryCourseData(course, coursePlayers, machines, players) {
    ...
    function joinCoursePlayersToHole(coursePlayers, hole) {
        ...
        coursePlayers.forEach((coursePlayer, cpIndex) => {
            let found = false;
            ...
            coursePlayer.holes.forEach((playerScore, index) => {
    ...
    const gameCourse = Object.assign({}, course);
    gameCourse.players = coursePlayers.map(coursePlayer => (Object.assign({ parType: gameCourse.parTypes.find(parType => coursePlayer.parTypeId == parType.uuid), player: players.find(p => p.uuid === coursePlayer.playerId) }, coursePlayer)));
    gameCourse.halfIndex = Math.floor((gameCourse.holes.length - 1) / 2);
    gameCourse.halfPar = 0;
    gameCourse.totalPar = 0;
    gameCourse.holes = gameCourse.holes.map((hole, holeIndex) => {
        /* course par totals */
        ...
        /* end */
        const holeClone = Object.assign({ machine: machines.find(machine => machine.uuid === hole.machineId) }, hole);
        joinCoursePlayersToHole(coursePlayers, holeClone);
        // add parType to pars
        for (let mPar in hole.pars) {
            holeClone.pars[mPar].parType = gameCourse.parTypes.find(parType => parType.uuid === mPar);
        }
        return holeClone;
    });
    gameCourse.players.forEach(player => {
        let halfSum = 0;
        for (let x = 0; x <= gameCourse.halfIndex; ++x) {
            const hole = gameCourse.holes[x];
            halfSum = halfSum + (hole.players[player.playerId] && hole.players[player.playerId].balls || 0);
        }
        let fullSum = halfSum;
        for (let x = gameCourse.halfIndex + 1; x <= gameCourse.holes.length - 1; ++x) {
            const hole = gameCourse.holes[x];
