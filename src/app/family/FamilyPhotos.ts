...
export class FamilyPhotos {
  ...
  loadPhotos(): Observable<any> {
    const collect = this.FamilyApp.db.collection(
      this.collectionBase,
      ref => ref.orderBy('added', 'desc')
    );

    const vc = collect.valueChanges();
    const ob = vc.pipe(
      leftJoinDocument(
        this.FamilyApp.db as any, // data typing compatibility because outdated firebase
        'addedBy',
        'users',
        'addedByUser'
      )
    ...
    ob.subscribe(
      data => data,
      err => log('FamilyPhotos', '#loadPhotos')
    );
